...
<div class="page-container">
	<form action="/goodslist/{{$data->id}}" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data" >
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>商品名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$data->name}}" placeholder="商品名称" id="" name="name">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="btn-upload form-group">
				...
				</span><img src="{{$data->pic}}" class="pd-20" width="100">(原始图片)<p id="mes" style="color:red;width:300px">图片留空默认不修改</p></div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				...
				@foreach ($cates as $v)
					<option value="{{$v->id}}" @if ($v->id == $data->cate_id) selected @endif>{{$v->name}}</option>
				@endforeach
				</select>
				</span></div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$data->num}}" placeholder="商品库存" id="" name="num">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">商品单价：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$data->price}}" placeholder="商品单价" id="" name="price">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">详细内容：</label>
			<div class="formControls col-xs-8 col-sm-9"> 
				<script id="editor" type="text/plain" name="descr" style="width:100%;height:400px;">{{$data->descr}}</script> 
			</div>
		</div>
		<div class="row cl">
        ...
        submitHandler:function(form){
            $('form').ajaxSubmit({
                ...
                success: function(data){
                    ...
                    layer.msg('文件格式不对!',{icon:2,time:1000});
                  }
                },
