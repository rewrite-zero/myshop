...
<div class="soubg">
    <div class="sou">
        <span class="fr"><a href="/homeindex" style="color:#ff4e00;">返回首页</a>&nbsp;
        <span class="fr">
            @if (session('name'))
	...
	<div class="m_content">
		...
		<div class="m_right">
            ...
            <table border="0" class="order_tab" style="width:930px; text-align:center; margin-bottom:30px;" cellspacing="0" cellpadding="0">
              <tr style="background-color:#ff4e00;color:white">                             
                <td width="20%">订单号</td>
                <td width="25%">下单时间</td>
              ...
              <tr style="background-color:#abcdef">
                <td data-toggle="modal" data-target="#myModal"><font color="#ff4e00">{{$v->order_num}}</font></td>
                <td>{{$v->addtime}}</td>
                <td>￥{{$v->total}}</td>
                <td>@if ($v->status == 0) 待付款 @elseif ($v->status == 1) 待发货 @elseif ($v->status == 2) 待收货 @elseif ($v->status == 3) 待评价 @elseif ($v->status == 4) 已评价 @endif </td>
                <td>
                    @if ($v->status == 0)
                    <a class="change" href="javascript:void(0)" name="/homeorder/{{$v->id}}/edit">付款</a>
                    @elseif ($v->status == 1)等待卖家发货
                    @elseif ($v->status == 2)<a class="change" href="javascript:void(0)" name="/homeorder/{{$v->id}}/edit">确认收货</a>
                    @elseif ($v->status == 3)<a class="change" href="javascript:void(0)" name="/homeorder/{{$v->id}}/edit">评价</a>
                  ...
                  @if ($vv['order_id'] == $v->id)
                  <tr>
                    <td><img src="{{$vv['goods']->pic}}" width="80" height="80"></td>
                    <td>{{$vv['goods']->name}}</td>
                    <td>x{{$vv['num']}}</td>
                    <td>单价￥{{$vv['goods']->price}}</td>
                    <td>总价￥{{$vv['total']}}</td>
                  </tr>
                  @endif
                  @endforeach
...
$('.change').click(function(){
    ...
    $.get(href,{},function(data){
        if (data == 1) {
            ...
            alert('收货成功');
            o.html('评价');
            o.parents('td').prev('td').html('待评价');
        } else if (data == 4) {
            alert('评价成功');
            p.html('订单完成');
            o.parents('td').prev('td').html('已评价');
        }
